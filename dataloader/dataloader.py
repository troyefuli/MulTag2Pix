...
def read_tagline_txt(tag_txt_path, img_dir_path, iv_dict, cv_dict, data_size=0, is_train=True, seed=-1):
    ...
    include_tags = [470575, 540830,1821,399836,412555,421662,449194,487156,479955,480899] # 1girl, 1boy
    ...
    for line in tag_lines:
        ...
        person_index = person_tag
       # # 修改后的    如果只有一个头发/眼睛  不要！
       #  if(person_tag != 470575 and person_tag != 540830):
       #      if (len(hair_tag) == 1 and len(eye_tag) == 1):
       #          awful_tag_num += 1
       #          if is_train:
       #              continue
                
#    一个tensor来储存 person人数             
        person_class = torch.zeros(10, dtype=torch.float)        
...
#         person 的处理            i
        for i in range(10):
            if person_index ==include_tags[i]:
                try:
                    person_class[i] = 1
                except IndexError as e:
                    raise e
        ...
        persons_list.append(person_class)
    ...
    return (file_id_list, persons_list, iv_class_list, cv_class_list)


class ColorAndSketchDataset(Dataset):
    def __init__(self, rgb_path, sketch_path_list, person_list, file_id_list, iv_class_list, cv_class_list,
    ...
    def __getitem__(self, idx):
        ...
        person_class = self.person_list[index]
        ...
        sketch_dir = random.choice(self.sketch_path_list)
        sketch_path = sketch_dir / f"{file_id}.png"

#        skeleton骨架  
        skeleton_dir = sketch_dir.with_name(f"{sketch_dir.name}_skeleton")
        skeleton_path = skeleton_dir / f"{file_id}.png"
        ...
        if self.both_transform is not None:
            color_img, sketch_img, skeleton_img = self.both_transform(color_img, sketch_img, skeleton_img)
...
# 返回的时候加了骨架
        return (color_img, sketch_img, person_class, skeleton_img, iv_tag_class, cv_tag_class)
...
class RandomFRC(transforms.RandomResizedCrop):
    ...
    def __call__(self, img1, img2, img3):
        ...
        img3 = tvF.resize(img3, self.size, interpolation=Image.LANCZOS)
        ...
        if random.random() < 0.5:
            ...
            img1 = tvF.rotate(img1, rot, Image.BILINEAR)
            img2 = tvF.rotate(img2, rot, Image.BILINEAR)
            img3 = tvF.rotate(img3, rot, Image.BILINEAR)
...
# 从tag_dump  把iv cv name 字典拿出来（全部） 还没分类的
    ...
    rgb_train_path = data_dir_path / "rgb_train_single"
#     keras_single_768
    sketch_dir_path_list = [ "keras_single_768"]
...
# 读tagline的文件  增加了train_person_list
    (train_id_list, train_person_list, train_iv_class_list, train_cv_class_list) = read_tagline_txt(
    ...
    train = ColorAndSketchDataset(rgb_path=rgb_train_path, sketch_path_list=sketch_dir_path_list,
        file_id_list=train_id_list, person_list=train_person_list, iv_class_list=train_iv_class_list, cv_class_list=train_cv_class_list,
    ...
    (test_id_list, test_person_list,test_iv_class_list, test_cv_class_list) = read_tagline_txt(
    ...
    test = ColorAndSketchDataset(rgb_path=rgb_test_path, sketch_path_list=[sketch_test_path],
        file_id_list=test_id_list, person_list=test_person_list, iv_class_list=test_iv_class_list, cv_class_list=test_cv_class_list,
...
class LinerTestDataset(Dataset):
    def __init__(self, sketch_path, file_id_list, iv_class_list, cv_class_list,
        ...
        self.skeleton_path = sketch_path.with_name(f"{sketch_path.name}_skeleton")
    ...
    def __getitem__(self, idx):
        ...
        skeleton_path = self.skeleton_path / f"{file_id}.png"
        ...
        return (sketch_img, file_id, skeleton_img, iv_tag_class, cv_tag_class)
...
def get_test_dataset(args):
    ...
    (test_id_list, _, test_iv_class_list, test_cv_clas_list) = read_tagline_txt(
